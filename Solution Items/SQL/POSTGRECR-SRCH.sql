...
BEGIN
	...
	TMP_NAME_RET_TBL := ARRAY(SELECT A.NAME_SET_ID FROM NAME_CMP_TBL A INNER JOIN NAME_CDTBL ON (A.NAME_CMP_VALUE = NAME_CDTBL.NAME_ID)
		WHERE 
			...
			(USE_VARIANTS_IN = TRUE AND NAME_MATCH_IN = 4 AND NAME_CMP_CLS_IN[1] = 1 AND UPPER(NAME_VALUE) IN (SELECT B.SYN_NAME FROM NAME_CMP_SYN_CDTBL B WHERE B.PRI_SOUNDEX = T_SOUNDEX AND STRENGTH >= 3))
				OR
