...
BEGIN
	...
	INSERT INTO PSN_VRSN_TBL 
		SELECT 
			...
			COALESCE(BRTH_CD_ID_IN, BRTH_CD_ID)
...
DECLARE
	...
	CTZN_ID_ID	DECIMAL;
BEGIN
	SELECT NTN_ID INTO NTN_ID_ID FROM NTN_TBL WHERE NTN_CS = NTN_CS_IN;
	IF NTN_ID_ID IS NULL THEN 
		NTN_ID_ID := nextval('NTN_SEQ');
		INSERT INTO NTN_TBL (NTN_ID, NTN_CS, NTN_NAME) VALUES (NTN_ID_ID, NTN_CS_IN, NTN_NAME_IN);
	END IF;
	CTZN_ID_ID := nextval('NTN_SEQ');
	INSERT INTO PSN_CTZNSHP_TBL (CTZN_ID, PSN_ID, NTN_ID, EFFT_TS_SET_ID, STATUS_CS, EFFT_VRSN_ID) VALUES
		(CTZN_ID_ID, PSN_ID_IN, NTN_ID_ID, EFFT_TS_SET_ID_IN, COALESCE(STATUS_CS_IN, 'Active'), EFFT_VRSN_ID);
	RETURN CTZN_ID_ID;
END
$$ LANGUAGE plpgsql;
...
BEGIN
	UPDATE PSN_CTZNSHP_TBL SET OBSLT_VRSN_ID = PSN_VRSN_ID ,
		STATUS_CS_IN = COALESCE(STATUS_CS_IN, 'Obsolete')
	WHERE
		PSN_ID = PSN_ID_IN AND NTN_ID = NTN_ID_IN;
END
$$ LANGUAGE plpgsql;
