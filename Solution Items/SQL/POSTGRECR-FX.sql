...
$$
BEGIN
	IF NOT IS_PRVT_IN AND EXISTS (SELECT PSN_ID FROM PSN_ALT_ID_TBL WHERE ID_DOMAIN = ID_DOMAIN_IN AND ID_VALUE = ID_VALUE_IN AND IS_HCN = TRUE) THEN
		RAISE EXCEPTION 'CR002: ALTERNATE IDENTIFIER ALREADY REGISTERED';
	ELSIF NOT IS_PRVT_IN AND EXISTS (SELECT PSN_ID FROM PSN_ALT_ID_TBL WHERE ID_DOMAIN = ID_DOMAIN_IN AND PSN_ID = PSN_ID_IN AND IS_HCN = TRUE) THEN
