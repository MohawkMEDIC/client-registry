...
(
	...
	TS_DATE		DATE NOT NULL, -- IDENTIFIES THE DATE WITHOUT TIMEZONE
...
CREATE SEQUENCE PLC_SEQ START WITH 1 INCREMENT BY 1;
...
(
	PLC_ID		DECIMAL(20,0) NOT NULL DEFAULT nextval('PLC_SEQ'), -- INTERNALLY GENERATED, LOCAL IDENTIFIER FOR THE SDL
	PLC_CLS_CS	CHAR(3) NOT NULL DEFAULT 'PLC' CHECK (PLC_CLS_CS IN ('PLC','SDL')), -- PLACE
	PLC_NAME	VARCHAR(255),  -- THE NAME OF THE SDL
	PLC_ADDR_SET_ID	DECIMAL(20,0), -- THE AD SET ID THAT REPRESENTS THE ADDRESS OF THIS SDL
	PLC_TYP_CD_ID	DECIMAL(20,0), -- THE CD ID THAT REPRESENTS THE TYPE OF THIS SDL
	CONSTRAINT PK_PLC_TBL PRIMARY KEY (PLC_ID),
	--CONSTRAINT FK_PLC_ADDR_SET_ID_ADDR_TBL FOREIGN KEY (PLC_ADDR_SET_ID) REFERENCES ADDR_CMP_TBL(ADDR_SET_ID), CANT DO THIS AS AN ADDR SET ID ISNT UNIQUE
	CONSTRAINT FK_PLC_TYP_CD_ID_CD_TBL FOREIGN KEY (PLC_TYP_CD_ID) REFERENCES CD_TBL(CD_ID)
);
...
(
	...
	GNDR_CS		VARCHAR(2), -- THE GENDER CODE OF THE PERSON AS OF THE CURRENT VERSION
	...
	BRTH_PLC_ID	DECIMAL(20,0), -- BIRTH PLACE
	ROL_CS		CHAR(3) NOT NULL DEFAULT 'PAT', -- ROLE
	...
	CONSTRAINT FK_PSN_BRTH_PLC_PLC_TBL FOREIGN KEY (BRTH_PLC_ID) REFERENCES PLC_TBL(PLC_ID)
);
...
-- @INDEX
-- LOOKUP BY CLASS
CREATE INDEX PSN_VRSN_ROL_CS_IDX ON PSN_VRSN_TBL(ROL_CS);
...
-- @INDEX
-- COLLATED INDEX FOR LIKE CONDITIONS
CREATE INDEX NAME_NAME_LIKE_VALUE_IDX ON NAME_CDTBL(NAME_VALUE varchar_pattern_ops);


-- @INDEX
-- COLLATED INDEX FOR LIKE CONDITIONS
CREATE INDEX NAME_NAME_LIKE_TXT_VALUE_IDX ON NAME_CDTBL(NAME_VALUE text_pattern_ops);
...
(
	...
	LNK_CLS		DECIMAL(12,0) NOT NULL, -- THE CLASSIFICATION OF THE LINK
...
--
CREATE TABLE PLC_ALT_ID_TBL
(
	PLC_ID		DECIMAL(20,0) NOT NULL, -- THE INTERNAL IDENTIFIER OF THE SDL THIS ALT ID BELONGS TO
	...
	CONSTRAINT PK_PLC_ALT_ID_TBL PRIMARY KEY (PLC_ID, ALT_ID_DOMAIN),
	CONSTRAINT FK_PLC_ID_PLC_TBL FOREIGN KEY (PLC_ID) REFERENCES PLC_TBL(PLC_ID)
);

-- @INDEX
-- NEED TO BE ABLE TO RESOLVE FROM A ID_DOMAIN+ID PAIR QUICKLY
CREATE UNIQUE INDEX PLC_ALT_ID_TBL_ALT_ID_IDX ON PLC_ALT_ID_TBL(ALT_ID_DOMAIN,ALT_ID);

-- @INDEX
-- NEED TO BE ABLE TO QUERY FROM PLC_ID QUICKLY
CREATE INDEX PLC_ALT_ID_TBL_PLC_ID_IDX ON PLC_TBL(PLC_ID);
...
--
CREATE TABLE HSR_PLC_TBL
(
	HSR_ID	DECIMAL(20,0) NOT NULL, -- IDENTIFIES THE HEALTH SERVICE RECORD TO WHICH THIS SDL ASSOC IS BOUND
	PLC_ID		DECIMAL(20,0) NOT NULL, -- IDENTIFIES THE SERVICE DELIVERY LOCATION TO WHICH THIS ASSOC IS BOUND
	PLC_CLS		DECIMAL(8) NOT NULL, -- IDENTIFIES THE CLASSIFICATION (OR HOW) OF THE ROLE LINK
	CONSTRAINT PK_HSR_PLC_TBL PRIMARY KEY (HSR_ID, PLC_ID, PLC_CLS),
	CONSTRAINT FK_HSR_ID_HSR_TBL FOREIGN KEY (HSR_ID) REFERENCES HSR_TBL(HSR_ID),
	CONSTRAINT FK_PLC_ID_PLC_TBL FOREIGN KEY (PLC_ID) REFERENCES PLC_TBL(PLC_ID)
);
...
CREATE VIEW HSR_PLC_VW AS
	SELECT HSR_PLC_TBL.HSR_ID, PLC_CLS, PLC_TBL.PLC_ID, PLC_NAME, PLC_ADDR_SET_ID, PLC_TYP_CD_ID
	FROM HSR_PLC_TBL INNER JOIN PLC_TBL USING (PLC_ID);
...
CREATE SEQUENCE CMP_TYP_SEQ START WITH 1 INCREMENT BY 1;

-- @TABLE
-- CREATE COMPONENT CODE TABLE
CREATE TABLE CMP_TYP_CDTBL
(
	CMP_TYP_ID	NUMERIC(4) NOT NULL DEFAULT nextval('CMP_TYP_SEQ'), -- COMPONENT TYPE ID
	CMP_CLS		VARCHAR(255) NOT NULL, -- COMPONENT TYPE
	CONSTRAINT PK_CMP_TYP_CDTBL PRIMARY KEY (CMP_TYP_ID)
);

-- UNIQUE CLASS 
CREATE UNIQUE INDEX CMP_TYP_CLS_IDX ON CMP_TYP_CDTBL(CMP_CLS);
...
(
	...
	CNTR_TYP_ID	NUMERIC(4) NOT NULL,
	CNTR_TBL_ID	DECIMAL(20,0) NOT NULL,
	CNTR_VRSN_ID	DECIMAL(20,0),
	CMP_TYP_ID	NUMERIC(4) NOT NULL,
	CMP_TBL_ID	DECIMAL(20,0) NOT NULL,
	CMP_VRSN_ID	DECIMAL(20,0), 
	CMP_ROL_TYP	DECIMAL(12,0) NOT NULL,
	CONSTRAINT PK_HSR_CMP_TBL PRIMARY KEY (CMP_ID),
	CONSTRAINT FK_HSR_CMP_CNTR_TYP_CMP_TYP_CDTBL FOREIGN KEY (CNTR_TYP_ID) REFERENCES CMP_TYP_CDTBL(CMP_TYP_ID),
	CONSTRAINT FK_HSR_CMP_CMP_TYP_CMP_TYP_CDTBL FOREIGN KEY (CMP_TYP_ID) REFERENCES CMP_TYP_CDTBL(CMP_TYP_ID)
);

-- @INDEX
-- LOOKUP COMPONENT BY VERSION ID SHOULD BE INDEXED
CREATE INDEX CMP_CNTR_IDX ON CMP_TBL(CNTR_TBL_ID);
CREATE INDEX CMP_CNTR_TYP_ID_IDX ON CMP_TBL (CNTR_TYP_ID, CNTR_TBL_ID, CNTR_VRSN_ID);

-- @INDEX
-- LOOKUP COMPONENT BY COMPONENT IDENTIFIER
CREATE INDEX CMP_CMP_IDX ON CMP_TBL(CMP_TBL_ID);
CREATE INDEX CMP_CMP_TYP_ID_IDX ON CMP_TBL (CMP_TYP_ID, CMP_TBL_ID, CMP_VRSN_ID);

-- @VIEW
-- COMPONENT TABLE BACKWARDS COMPATIBILITY TABLE
CREATE VIEW CMP_VW AS
	SELECT CMP_ID, A.CMP_CLS AS CNTR_TYP, CNTR_TBL_ID, CNTR_VRSN_ID, B.CMP_CLS AS CMP_TYP, CMP_TBL_ID, CMP_VRSN_ID, CMP_ROL_TYP FROM
		CMP_TBL INNER JOIN CMP_TYP_CDTBL AS A ON (CMP_TBL.CNTR_TYP_ID = A.CMP_TYP_ID)
		INNER JOIN CMP_TYP_CDTBL AS B ON (CMP_TBL.CMP_TYP_ID = B.CMP_TYP_ID);
-- @SEQUENCE
-- CUSTODIAL DEVICE ID
...
-- @TABLE
-- ETHNIC GROUP CODE
CREATE TABLE PSN_ETH_GRP_TBL
(
	PSN_ID		DECIMAL(20,0) NOT NULL, -- THE VERSION OF THE CLIENT RECORD TO WHICH THIS RACE APPLIES		
	ETH_GRP_CD_ID	DECIMAL(20,0) NOT NULL, -- THE ETHNIC GROUP CODE FOR THE PERSON RECORD
	EFFT_VRSN_ID	DECIMAL(20,0) NOT NULL, -- THE VERSION IDENTIFIER WHERE THIS DATA BECAME EFFECTIVE
	OBSLT_VRSN_ID	DECIMAL(20,0), -- THE VERSION IDENTIFIER WHERE THIS DATA BECAME OBSOLETE
	CONSTRAINT PK_PSN_ETH_GRP_TBL PRIMARY KEY (PSN_ID, ETH_GRP_CD_ID),
	CONSTRAINT FK_PSN_ETH_GRP_PSN_TBL FOREIGN KEY (PSN_ID) REFERENCES PSN_TBL(PSN_ID),
	CONSTRAINT FK_PSN_ETH_GRP_EFFT_VRSN_TBL FOREIGN KEY (EFFT_VRSN_ID) REFERENCES PSN_VRSN_TBL(PSN_VRSN_ID),
	CONSTRAINT FK_PSN_ETH_GRP_OBSLT_VRSN_TBL FOREIGN KEY (OBSLT_VRSN_ID) REFERENCES PSN_VRSN_TBL(PSN_VRSN_ID),
	CONSTRAINT FK_PSN_ETH_GRP_CD_ID_CD_TBL FOREIGN KEY (ETH_GRP_CD_ID) REFERENCES CD_TBL(CD_ID)
	
);

-- @INDEX LOOKUP ETHNIC GROUP BY PERSON
CREATE INDEX ETH_GRP_PSN_ID_IDX ON PSN_ETH_GRP_TBL(PSN_ID);

-- @INDEX LOOKUP BY ETHNIC GROUP
CREATE INDEX ETH_GRP_ETH_GRP_CD_IDX ON PSN_ETH_GRP_TBL(ETH_GRP_CD_ID);
