...
-- CLEAN TABLES
DROP TABLE DEV_TBL CASCADE;
DROP TABLE NAME_CDTBL CASCADE;
...
DROP TABLE PSN_CTZNSHP_TBL CASCADE;
DROP TABLE NTN_TBL CASCADE;
DROP TABLE PSN_TBL CASCADE;
DROP TABLE EXT_KEY_TBL CASCADE;
...
-- @TABLE
-- NATIONS TABLE
CREATE TABLE NTN_TBL
(
	NTN_ID		DECIMAL(10,0) NOT NULL DEFAULT nextval('NTN_SEQ'), -- UNIQUE IDENTIFIER FOR THE NATION
	NTN_CS		CHAR(3) NOT NULL, -- ISO3166-1 CODE FOR THE NATION
	NTN_NAME	VARCHAR(200) NOT NULL, -- NATION NAME
	CONSTRAINT PK_NTN_TBL PRIMARY KEY (NTN_ID),
	CONSTRAINT FK_NTN_TYP_CD_TBL FOREIGN KEY (NTN_CD_ID) REFERENCES CD_TBL(CD_ID)
);


-- @INDEX
-- LOOKUP NATION BY CODE
CREATE UNIQUE INDEX NTN_CD_ID_IDX ON NTN_TBL(NTN_CS);
...
(
	...
	NTN_ID		DECIMAL(10,0) NOT NULL, -- IDENTIFIES THE NATION TO WHICH THE PERSON IS A CITIZEN
	...
	CONSTRAINT FK_PSN_CTZNSHP_NTN_TBL FOREIGN KEY (NTN_ID) REFERENCES NTN_TBL(NTN_ID),
...
CREATE UNIQUE INDEX NAME_NAME_VALUE_IDX ON NAME_CDTBL(NAME_VALUE);
...
(
	...
	DEV_EXT 	VARCHAR(255) NOT NULL, -- EXTENSION IDENTIFIER FOR DEVICE
